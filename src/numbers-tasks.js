...
function getFibonacciNumber(index) {
  ...
  for (let i = 2; i <= index; i += 1) {
    const newValue = prevValue + curValue;
    prevValue = curValue;
    curValue = newValue;
  }
  if (index === 0) {
...
function getSumOfDigits(num) {
  const numOfString = String(num);
  const sum = numOfString.split('').reduce((acc, current) => {
...
function isNumber(number) {
  return !!(Number(number) === number && Number.isFinite(number));
}
...
function getCountOfOddNumbers(number) {
  let count = 0;
  for (let i = 0; Math.abs(number) > i; i += 1) {
